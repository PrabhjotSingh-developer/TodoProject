...
const InnerTodo = ({ item }) => {
  ...
  function updateTodo() {
    if (!isEditable) {
      setIsEditable(true);
      toast.warn("Save this task after Editing");
    } else if (content != "") {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex items-center gap-4 px-[15px] grow-[1]">
          ...
          />
          {
            <input
              type="text"
              className={`bg-transparent outline-none px-2 w-[100%] py-1 text-font  ${
                isEditable
                  ? "border-b-slate-600 border-2 border-solid "
                  : "border-none"
      ...
      <div className="flex gap-7 flex-col items-center text-font w-[100%] ">
        ...
        >
          <button
            className="border-top flex gap-2 items-center  "
            onClick={updateTodo}
          >
            {isEditable ? (
              <>
                <FaSave className="text-[20px] w-[24px]" />
                Save Data
              </>
            ) : (
              <>
                <GoPencil className="text-[20px] w-[24px]" />
                Edit Todo
              </>
            )}
          </button>
        ...
        <div
          ...
          style={{ borderTop: "1.5px solid #496E4B" }}
        >
          <button className="border-top gap-2 items-center flex ">
            {" "}
            <CiBellOn className="text-[20px] w-[24px] " /> Set Reminder
          </button>
        </div>
        <div
          className="px-8 py-2  w-full"
        ...
        >
          <button
            className="border-top gap-1 items-center text-[25px]"
            onClick={() => dispatch(toggleMiniTodo({ isOpen: false, item }))}
          >
            {" "}
            <RxCross2 />
          </button>
          <button className=" text-[25px]" onClick={deleteTodoItem}>
            <MdDelete />
          </button>
        </div>
      </div>
